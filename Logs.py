...
query1 = "SELECT title, COUNT(*) AS views FROM articles , log  WHERE articles.slug = SUBSTR(log.path, 10) GROUP BY title ORDER BY views DESC LIMIT 3; "

Question2 = "2. Who are the most popular article authors of all time?"
query2 = "SELECT name, COUNT(*) AS views FROM authors , articles , log WHERE articles.slug = SUBSTR(log.path,10) AND articles.author = authors.id GROUP BY name ORDER BY views DESC;"
